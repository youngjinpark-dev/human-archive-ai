...
- 심층 인터뷰 (동적 9~30질문, why→how→what-if 연쇄, 포화도 기반 종료)
- 음성 파일 업로드 (STT + 임베딩)
- YouTube 자막, 문서 등 멀티소스 지식 아카이빙
- RAG 기반 전문가 응답 생성

### 판단 프레임워크 아카이빙
- 심층 인터뷰/음성에서 판단 축, if-then 패턴, 경험 스토리 자동 추출
- 판단 자문: "이 전문가라면 어떻게 판단할까?" + 근거 스토리
- 유사 경험 스토리 검색 (RAG 기반)
- 두 접근법 전문가 관점 비교 분석
...
| `interview` | 인터뷰로 페르소나 구축 (classic: 9개 질문, deep: 동적 심층) |
...
| `consult_judgment` | 전문가 판단 프레임워크 기반 상황 자문 |
| `get_framework` | 전문가의 판단 프레임워크 조회 |
| `find_similar_story` | 유사 경험 스토리 검색 |
| `compare_approaches` | 두 접근법 전문가 관점 비교 |
...
| GET | `/api/external/framework` | 프레임워크 조회 |
| POST | `/api/external/stories` | 유사 경험 검색 |
...
src/
  ...
  components/        # 공통 컴포넌트
  lib/               # 유틸리티 (LLM, 임베딩, 판단 추출, 심층 인터뷰 등)
  types/             # TypeScript 타입
mcp/                 # MCP 서버
supabase/            # DB 마이그레이션
docs/                # 기획 문서
